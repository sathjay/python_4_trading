...
LSTM_ML_LO = html.Div([
    html.Br(),
    html.P("Here users can see how well a Multi Layer (4 layer) Long Short Term Memory (LTSM) Recurring Neural Network (RNN) model\
    ...
    html.Label('Select the Input features for Multi Layer LSTM model: (By default Close price will be an input feature. If None selected, then Close Price will be input feature.)'),
    ...
    html.Br(),
    html.H4('Select the LSTM RNN Multi Layer model parameters: '),
    ...
    html.Button('Submit', id='BT_LSTM_ML',
                className='button', n_clicks=0),

    html.Hr(),
    dcc.Loading(children=[
        html.Div(id='message_ML'),
        ...
        html.Hr(),
        dcc.Graph(id='price_chart_LSTM_Multi_Layer', figure=blank_fig(),
                  style={'width': '92%', 'height': '74vh'}),

        html.Hr(),
        dcc.Graph(id='daily_price_change_LSTM_Multi_Layer', figure=blank_fig(),
                  style={'width': '92%', 'height': '74vh'}),

        html.Hr(),
        html.Div(
            dash_table.DataTable(id='accuracy_ML',
                                 style_cell={
                                     'textAlign': 'center'},
                                 style_header={'backgroundColor': 'blue',
                                               'fontWeight': 'bold',
...
@app.callback(
    [Output('message_ML', 'children'),
     Output('model_summary_ML', component_property='data'),
     Output('model_summary_ML', component_property='columns'),
     Output('price_chart_LSTM_Multi_Layer', 'figure'),
     Output('daily_price_change_LSTM_Multi_Layer', 'figure'),
     Output('accuracy_ML', component_property='data'),
     Output('accuracy_ML', component_property='columns'),],
    [Input('BT_LSTM_ML', 'n_clicks')],
...
def LTSM_Multivariate_Multi_Layer_model(n_clicks, selected_stock, lookback_days, batch_size, activation, additional_feature):
    ...
    # Date in the input column is dropped.
    nuerons = int((backcandles) * (len(input_features)-1))

    model = Sequential()
    model.add(LSTM(nuerons, name='LSTM_First', activation=activation, input_shape=(
        X_train.shape[1], X_train.shape[2]), return_sequences=True))
    model.add(Dropout(0.1))
    model.add(LSTM(int(nuerons/2), activation=activation,
              name='LSTM_Second', return_sequences=True))
    model.add(Dropout(0.1))
    model.add(LSTM(int(nuerons/4), activation=activation,
              name='LSTM_Third', return_sequences=True))
    model.add(Dropout(0.1))
    model.add(LSTM(int(nuerons/8), activation=activation,
              name='LSTM_Fourth', return_sequences=False))
    ...
    price_chart_LSTM_Multi_Layer_Data = []
    ...
    price_chart_LSTM_Multi_Layer_Data.append(trace1)
    ...
    price_chart_LSTM_Multi_Layer_Data.append(trace2)
    ...
    price_chart_LSTM_Multi_Layer_Data.append(trace3)

    price_chart_LSTM_Multi_Layer_Layout = go.Layout(
    ...
    fig = go.Figure(data=price_chart_LSTM_Multi_Layer_Data,
                    layout=price_chart_LSTM_Multi_Layer_Layout)
    ...
    return message_ML, model_summary_data, model_summary_col, fig, fig1, accuracy_data, accuracy_col
