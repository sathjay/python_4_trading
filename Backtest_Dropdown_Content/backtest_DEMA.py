...
DEMA_LO = html.Div([
    ...
    html.H5('Double Exponential Moving Average (DEMA) Crossover Strategy:',
            className='content_title'),

    html.P('In the DEMA Crossover strategy:'),
    ...
    html.P('In essence, DEMA is designed to be more responsive than the EMA and reduce the lag inherent in all moving averages. While this can be an advantage in fast-moving markets, it also means that DEMA might produce more false signals in volatile conditions compared to the EMA.'),
    ...
    dcc.Loading(children=[
        ...
        html.Label(id='Selected_Value_DEMA'),

        html.Div(id='DEMA_note', children=[]),
...
@app.callback(
    [Output('Selected_Value_DEMA', 'children'),
        Output('DEMA_note', 'children'),
     Output('entry_and_exit_DEMA', component_property='figure'),
     Output('pnl_chart_DEMA', component_property='figure')],
    [Input('BT_DEMA', 'n_clicks')],
    [
...
def caluculate_double_expo_moving_average_payoff(n_clicks, selected_stock, FMA, SMA, backtest_days):
    ...
    if SMA <= FMA:
        ...
        fig1 = blank_fig()
        DEMA_note = ''
        return message, DEMA_note, fig, fig1

    else:
        message = html.P("You have selected {} days for Fast moving average and {} days for Slow moving average".format(FMA, SMA),
                         style={'color': 'black', 'font-size': '18px'})

        DEMA_note = html.P(["Assumption: At start, if the Fast moving average is greater than the slow moving average then trader takes the long postion and if the Fast moving average is less than the slow moving average then trader takes the short postion."], className='note'
                           )
        ...
        dff['Fast_DEMA'] = (2*dff['Fast_EMA']) - \
            dff['Fast_EMA'].ewm(span=FMA, adjust=False).mean()
        dff['Slow_DEMA'] = (2*dff['Slow_EMA']) - \
            dff['Slow_EMA'].ewm(span=SMA, adjust=False).mean()
        ...
        dff1['combined_signal'] = np.where(
            dff1['Fast_DEMA'] > dff1['Slow_DEMA'], 1, 0)
        dff1['combined_signal'] = np.where(
            dff1['Fast_DEMA'] < dff1['Slow_DEMA'], -1, dff1['combined_signal'])
        ...
        dff1['signal'] = np.where(dff1['Fast_DEMA'] > dff1['Slow_DEMA'], 1, 0)
        ...
        if dff1['Fast_DEMA'][1] > dff1['Slow_DEMA'][1]:
            dff1['Buy'][1] = dff1['Close Price'][1]
        else:
            dff1['Sell'][1] = dff1['Close Price'][1]

        # dff1.to_csv('Double_expo_moving_avg.csv',mode = 'w',sep = '|')

        column_list = ['Close Price', 'Fast_DEMA', 'Slow_DEMA']
        price_chart_data = []
